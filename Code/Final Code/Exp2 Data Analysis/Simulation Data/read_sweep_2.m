%% Read Simulation and Experimental Acceleration Data for Experiment 2

for i=6:6 % Range from 1 to 9
    ...
    T1 = readtable('/Users/jacobsony/Desktop/try4/a_sim_' + string(f) + '.csv');
    T2 = readtable('/Users/jacobsony/Desktop/try5/a_sim_' + string(f) + '.csv');
    T3 = readtable('/Users/jacobsony/Desktop/try6/a_sim_' + string(f) + '.csv');
    T4 = readtable('/Users/jacobsony/Documents/IITB/RIYA/RIYA Project/Code/Dynamic/Final/Exp2 Data Analysis/set' + string(i) + '.xlsx','Sheet','Vibration');
    ...
    hold on
    plot(t4 - start_t4 + 0.65*(1/f), a4_b,'LineWidth',2,'Color','#FFA500','LineStyle',':')
    hold on
    plot(t - start_t,a1_m,'LineWidth',2,'Color','Blue')
    hold on
    plot(t - start_t, a3_m,'LineWidth',2,'Color','Blue','LineStyle','-.')
    hold on
    plot(t - start_t, a2_m,'LineWidth',2,'Color','Green','LineStyle',':')
    hold on
    plot(t4 - start_t4 + 0.65*(1/f), a4_m,'LineWidth',2,'Color','Magenta')
    ...
    ylim([min_y-0.1*abs(min_y),max_y+ 0.8*abs(max_y)])
    xlabel('$t$ [s]',"Interpreter","latex","FontSize",15)
    ylabel('$\ddot{x}$ [mm/s\textsuperscript{2}]',"Interpreter","latex","FontSize",15)
    set(gca, 'FontSize',15)

    % Export plot
    plotpath = fullfile('/Users/jacobsony/Desktop','pic2.pdf'); % Use appropriate folder path
    exportgraphics(T,plotpath,'Resolution',1000)
