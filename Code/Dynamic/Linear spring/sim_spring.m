...
x0 = [0;1;x_base_init;0];

%% Solve the ODEs to obtain the states numerically 

options=odeset('abstol',1e-9,'reltol',1e-9);
[t,x] = ode45(@(t,x)dyn_spring(t,x,m,g,b),tspan,x0,options);
...
for i=1:100:length(tspan)
    subplot(2,2,[2 4])
    plot([0,0],[x_base_init - x(i,3),l0 - x(i,1)],'LineWidth',3,'Color','Blue')
    hold on
    rectangle('Position',[-0.3 l0-x(i,1) 0.6 0.6],'LineWidth',3,'FaceColor','#853A00')
    hold on
    rectangle('Position',[-1 x_base_init-x(i,3)-0.3 2 0.3],'LineWidth',3,'FaceColor','#808090')
    hold off
    axis([-2,2,-2,10])
...
En_arr = 0.5 * m * x(:,2).^2 - m * g * x(:,1) + 0.5 * 155 * (x(:,1) - x(:,3)).^2;
En_arr2 = 0.5 * m * x(:,2).^2 - m * g * x(:,1) + x(:,4);


figure;
set(gcf,'defaultAxesTickLabelInterpreter','latex'); 
plot(t, En_arr,'LineWidth',2,'Color','Blue')
hold on
plot(t, En_arr2,'LineWidth',2,'Color','Red','LineStyle',':')
